    ...
    return (
        <ThemeProvider theme={theme}>
            <div style={{ padding: "20px", marginLeft: "-20px" }}>
                {!submitted ? (
                    <form ref={formRef} onSubmit={handleSubmit} className="bg-indigo-900 p-8 rounded-lg shadow-md">
                        ...
                        <textarea id="volunteer-message" name="message" rows="4" required className="w-full px-4 py-2 mb-4 border rounded-md text-white font-bold italic"></textarea>
                        <button type="submit" className="w-full bg-white hover:bg-indigo-400 text-indigo-900 font-bold py-2 px-4 rounded-md cursor-pointer">Submit</button>
                    </form>
                ) : (
                    <p className="text-white font-bold italic">Thank you for volunteering!</p>
                )}
            </div>
        </ThemeProvider>
    );
